...
def normalcheck(n):
    ...
    return ((n[0] * l[0] + n[1] * l[1] + n[2] * l[2]) / (
            math.sqrt(n[0] ** 2 + n[1] ** 2 + n[2] ** 2) * math.sqrt(l[0] ** 2 + l[1] ** 2 + l[2] ** 2))) < 0
def R(a, b, c):
    return (np.array([[1, 0, 0], [0, cos(a), sin(a)], [0, -sin(a), cos(a)]]) @ np.array(
        [[cos(b), 0, sin(b)], [0, 1, 0],
         [-sin(b), 0, cos(b)]]) @ np.array(
...
def endzone(x0, y0, z0, x1, y1, z1, x2, y2, z2):
    ...
    m0 = (r @ np.array([x0, y0, z0])) + np.array([0.001, -0.03, 0.2])
    m1 = (r @ np.array([x1, y1, z1])) + np.array([0.001, -0.03, 0.2])
    m2 = (r @ np.array([x2, y2, z2])) + np.array([0.001, -0.03, 0.2])

    xs0 = m0[0] * 1800 / m0[2] + 1000
    ys0 = m0[1] * 1800 / m0[2] + 1000
    xs1 = m1[0] * 1800 / m1[2] + 1000
    ys1 = m1[1] * 1800 / m1[2] + 1000
    xs2 = m2[0] * 1800 / m2[2] + 1000
    ys2 = m2[1] * 1800 / m2[2] + 1000
    ...
    if normalcheck(n):
        ...
        for i in range(xmin, xmax + 1):
            for j in range(ymin, ymax + 1):
                if barcordcheck(i, j, xs0, ys0, xs1, ys1, xs2, ys2):
                    zb = -zboof(i, j, xs0, ys0, z0, xs1, ys1, z1, xs2, ys2, z2)
                    if zb < z[i][j]:
                        z[i][j] = zb
...
for k in range(len(f)):
    ...
    y2 = v[f[k][2] - 1][1]
    z2 = v[f[k][2] - 1][2]
    endzone(x0, y0, z0, x1, y1, z1, x2, y2, z2)
